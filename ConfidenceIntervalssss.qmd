...
rnorm() = normal distribution random
rbeta() - beta distribution random
...
## CRITICAL VALUES
We will often use q_() function to get the value of X associated with particular quantiles of a distribution, whether that distribution is theoritcal or empirical
```{r}
...
What is the theoritical 95% CI around the estimate of the mean of the following vector
```{r}
...
Instead of assuming its normal, resample, use data from a single sample and we use monte carlo to approximate a sampling distribution even without access to the population from which samples are drawn
```{r}
